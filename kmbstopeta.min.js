    ...
    link = "https://data.etabus.gov.hk/v1/transport/kmb/eta/" + t + "/" + n + "/" + a, fetch(link).then(e => e.json()).then(
        t => {
            for (var a, l = 0; l < t.data.length; l++)
                ...
                min = Math.round((timeEta - timeNow) / 1e3 / 60), min < 1 && (min = "<1")), a.rmk_tc) {
                    case "原定班次":
                        remark = "原定班次";
                        break;
                    case "最後班次":
                    ...
                    case "服務只限於星期日及公眾假期":
                        ...
                        break;
                    case "服務只限於星期六、日及公眾假期":
                        remark = '<marquee style="color:red">服務只限於星期六、日及公眾假期</marquee>';
                        break;
                default:
                    remark = a.rmk_tc
            }
            null == a.eta && "" == remark && (remark = '<div style="color:red">暫時沒有原定班次</div>'), document.getElementsByClassName(
